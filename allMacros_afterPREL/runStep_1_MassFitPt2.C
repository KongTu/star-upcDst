...
void runStep_1_MassFitPt2(const bool doSys_ = false, TString sys_name="yieldExtraction", const bool draw_ = true){
	...
	TH1D* JpsiPt2_binCounting = new TH1D("JpsiPt2_binCounting","JpsiPt2_binCounting",pt2Nbins,pt2bins);
	TH1D* JpsiPt2_zdc = new TH1D("JpsiPt2_zdc","JpsiPt2_zdc",pt2Nbins,pt2bins);
	TH1D* JpsiPt2_zdc_binCounting = new TH1D("JpsiPt2_zdc_binCounting","JpsiPt2_zdc_binCounting",pt2Nbins,pt2bins);
	...
	for(int i = 0; i < Nbins+1; i++){
		...
		int count = 0;
		while( (!r->IsValid() || r->CovMatrixStatus()!= 3) && count < 20 ) {
		...
		JpsiPt2_binCounting->SetBinContent(i+1, signal_yield);
		JpsiPt2_binCounting->SetBinError(i+1, signal_error);
		
		TLegend *w4 = new TLegend(0.2,0.55,0.55,0.75);
		...
		TH1D* JpsiMassZDCForCounting = (TH1D*) hJpsiMassZDC_Pt2_1D[i]->Clone(Form("JpsiMassZDCForCounting_%d",i));
		total_yield_counting = JpsiMassZDCForCounting->Integral(JpsiMassZDCForCounting->FindBin(2.0), JpsiMassZDCForCounting->FindBin(3.5));
		signal_yield = total_yield_counting - fitJBkg_zdc[i]->Integral(2.0,3.5);
		signal_error = sqrt(total_yield_counting - fitJBkg_zdc[i]->GetParError(3)*fitJBkg_zdc[i]->GetParError(3));
		JpsiPt2_zdc_binCounting->SetBinContent(i+1, signal_yield);
		JpsiPt2_zdc_binCounting->SetBinError(i+1, signal_error);
	...
	for(int i = Nbins; i < Nbins+1; i++){
		...
		cout << "binCounting total yield: "<< signal_yield << endl;
		cout << "binCounting total yield error: "<< signal_error << endl;

		TLatex* r5011 = new TLatex(0.47,0.73, Form("signal bin count. yield = %.0f #pm %.0f", signal_yield, signal_error));
		r5011->SetNDC();
		r5011->SetTextSize(20);
		r5011->SetTextFont(44);
		// if( draw_) r5011->Draw("same");
	...
	JpsiPt2_binCounting->SetMarkerStyle(24);
	JpsiPt2_binCounting->Draw("Psame");

	JpsiPt2_zdc->SetMarkerStyle(21);
	JpsiPt2_zdc->Draw("Psame");

	JpsiPt2_zdc_binCounting->SetMarkerStyle(25);
	JpsiPt2_zdc_binCounting->Draw("Psame");
	...
	if( doSys_ ) outputname = "systematics-input/"+sys_name+"/output-Step_1-"+sys_name+"_"+name_of_template+".root";
	TFile outputfile(outputname,"RECREATE");
	JpsiPt2->Write();
	JpsiPt2_binCounting->Write();
	JpsiPt2_zdc->Write();
	JpsiPt2_zdc_binCounting->Write();

	c1_2->Print("../paper/figures/Figure_02_a.pdf");
	c1_2->Print("../paper/figures/Figure_02_a.png");
	if( doSys_) c2->Print("systematics-figures/"+sys_name+"/fit-signal-"+sys_name+"_"+name_of_template+".pdf");
