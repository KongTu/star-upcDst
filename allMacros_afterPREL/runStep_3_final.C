...
void runStep_3_final( const bool doSys_ = false ){
    ...
    cout << fCoh->Integral(0,1.2) << ";" << fCoh->IntegralError(0,1.2,params_coh,covMatrix_coh.GetMatrixArray(),1e-2) << " ; ";
    cout << fIncoh->Integral(0,1.2) <<  ";" << ((fDisso->IntegralError(0,1.2,params_disso,covMatrix_disso.GetMatrixArray(),1e-2)) / fDisso->Integral(0,1.2) )*fIncoh->Integral(0,1.2) << " ; ";
    cout << fDisso->Integral(0,1.2) <<  ";" << fDisso->IntegralError(0,1.2,params_disso,covMatrix_disso.GetMatrixArray(),1e-2) <<  " ; ";
    double x[1] = {0.};
	double err[1];
	r2[4]->GetConfidenceIntervals(1, 1, 1, x, err, 0.683, false);

    cout << fCoh->Eval(0) << ";" << err[0] <<  " ; ";
    cout << fIncoh->Eval(0) << ";" << func2->GetParError(4)*ratioIncohToDissco <<  " ; ";
    cout << fDisso->Eval(0) << ";" << func2->GetParError(4) <<  " ; ";
    cout << fCoh->Integral(0,1.2) / (fCoh->Integral(0,1.2) + fIncoh->Integral(0,1.2) + fDisso->Integral(0,1.2)) <<  " ; ";
    cout << func2->Integral(0,1.2) << ";" << func2->IntegralError(0.,1.2) <<  " ; ";
    cout << endl;
