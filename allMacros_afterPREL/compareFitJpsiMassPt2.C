...
void compareFitJpsiMassPt2( bool draw_ = false){

	TFile* file[3];

	TString default_name = "RawJpsiPt2-template_86.root";
	TString sys_name_1   = "RawJpsiPt2-signal-bincounting_86.root";
	TString sys_name_2   = "RawJpsiPt2-signal-bincounting_86.root";
	...
	for(int ifile=0;ifile<3;ifile++){
		histo[ifile] = (TH1D*) file[ifile]->Get("JpsiPt2");
	}
	...
	TH1D* base1 = makeHist("base1", "", "-t #approx p^{2}_{T, J/#psi} (GeV^{2})", "ratio of dN/dt", 100,0,3,kBlack);
	base1->GetYaxis()->SetRangeUser(0.75, 1.5);
	...
	h_ratio_sys_1->SetMarkerStyle(20);
	h_ratio_sys_2->SetMarkerStyle(21);
	...
	if(draw_) c1->Print("systematics-figures/signal_bincounting/figure-ratio.pdf");
