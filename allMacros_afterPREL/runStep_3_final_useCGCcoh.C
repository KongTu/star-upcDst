...
Double_t fitTotalCrosSection(Double_t *x, Double_t *par){
	...
	double incoherent = (ratioIncohToDissco*par[2])*TMath::Exp(par[1]*x[0]);
...
void runStep_3_final_useCGCcoh( const int which_coh = 0 ){
    ...
    //prediction from Hekki and Bjoren
    TGraphErrors* incoh_fluc_cgc = (TGraphErrors*) file_cgc->Get("incoh_fluc_cgc");
    TGraphErrors* coh_cgc = (TGraphErrors*) file_cgc->Get("coh_cgc");

    //total fit
	TF1 *func2 = new TF1("func2",fitTotalCrosSection,0,2.5,5);
	func2->SetParameter(0, 900);
	func2->FixParameter(1, -4.3);
	...
	//first fit
	TFitResultPtr r = hTotal->Fit("func2","RMES0+","",fit_low,fit_high);
	TGraphErrors* gr[5];  
	TFitResultPtr r2[5];
	...
	base1->Draw();
	TF1* fCoh = new TF1("fCoh",fitDFF,0,2.0,1);
	fCoh->SetParameter(0, func2->GetParameter(0));
	fCoh->SetLineColor(kBlue);
	fCoh->SetLineWidth(3);
	fCoh->Draw("Lsame");

	TF1* fIncoh = new TF1("fIncoh","[1]*TMath::Exp([0]*x[0])",0,2.0);
	fIncoh->SetParameter(0, func2->GetParameter(1) );
	fIncoh->SetParameter(1, func2->GetParameter(2)*ratioIncohToDissco );
	fIncoh->SetParError(0, func2->GetParError(1) );
	fIncoh->SetParError(1, func2->GetParError(2)*ratioIncohToDissco );
    ...
    cout << fIncoh->Integral(integral_low,integral_high) <<  ";" << ((fDisso->IntegralError(integral_low,integral_high,params_disso,covMatrix_disso.GetMatrixArray(),1e-2)) / fDisso->Integral(integral_low,integral_high) )*fIncoh->Integral(integral_low,integral_high) << " ; ";
    ...
    cout << fIncoh->Eval(0) << ";" << func2->GetParError(2)*ratioIncohToDissco <<  " ; ";
    ...
    // c1->Print("Preliminary/Figure_03_main_cgcCoh"+coh_name+".pdf");
