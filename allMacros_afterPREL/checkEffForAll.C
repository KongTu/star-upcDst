...
void checkEffForAll(){
	...
	for(int ik=0;ik<hSingleTrackGEN->GetNbinsX();ik++){
		double Ea = hSingleTrackREC->GetBinContent(ik+1);
		double Eb = hSingleTrackGEN->GetBinContent(ik+1);

		double value = Ea / Eb;

		double Ea_e = hSingleTrackREC->GetBinError(ik+1);
		double Eb_e = hSingleTrackGEN->GetBinError(ik+1);
		
		double bincenter = hSingleTrackGEN->GetBinCenter(ik+1);
		gr->SetPoint(ik, bincenter, value);
		gr->SetPointError(0, calColError(Ea, Eb, Ea_e, Eb_e) );
	}
	...
	TH1D* effratio = (TH1D*) file1->Get("effratio");
	...
	TLegend *w1 = new TLegend(0.22,0.17,0.43,0.27);
	w1->SetLineColor(kWhite);
	w1->SetFillColor(0);
	w1->SetTextSize(20);
	w1->SetTextFont(45);
	w1->AddEntry(gr2, "J/#psi eff only ", "P");
	w1->AddEntry(h_eff, "total J/#psi eff ", "P");
	w1->Draw("same");
	...
	TH1D* base3 = (TH1D*) base1->Clone("base1");
	base3->GetYaxis()->SetTitle("J/#psi eff");
	base3->GetXaxis()->SetTitle("p^{2}_{T} (GeV^{2})");
	base3->GetXaxis()->SetRangeUser(0,2.0);
	base3->GetYaxis()->SetRangeUser(0,0.5);
	base3->Draw("");

	effratio->SetMarkerStyle(20);
	effratio->Draw("PE same");

	TLegend *w2 = new TLegend(0.28,0.57,0.53,0.6);
	w2->SetLineColor(kWhite);
	w2->SetFillColor(0);
	w2->SetTextSize(20);
	w2->SetTextFont(45);
	w2->AddEntry(effratio, "total J/#psi eff RooUnfoldBayes ", "L");
	w2->Draw("same");

	c3->Print("~/Desktop/totalJpsiEff.pdf");
